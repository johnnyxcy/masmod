...
############################################################
# File: masmod/masmod/symbols/_sigma.py
#
# Author: Chongyi Xu <johnny.xcy1997@outlook.com>
#
# File Created: 11/28/2022 08:52 pm
#
...
def sigma(
    ...
    Args:
        ...
        bounds (list[BoundsType] | np.ndarray | None, optional): 每个 eps 的上下限，默认所有的 eps 都为 (-inf, inf)
        fixed (bool, optional): 是否固定，如果为 True 整个下三角矩阵构建的 Sigma 都会固定，默认为 False

    Returns:
        list[Eps]: 生成的多个 eps 对象
    """
    ...
...
@typing.overload
def sigma(
    init_sigma: list[list[ValueType]] | np.ndarray,
    bounds: list[BoundsType | None] | np.ndarray | None = None,
    fixed: bool = False,
) -> list[Eps]:
    ...
    Args:
        init_sigma (list[list[ValueType]] | np.ndarray): 完整 Sigma 矩阵，必须为方块矩阵
        bounds (list[BoundsType] | np.ndarray | None, optional): 每个 eps 的上下限，默认所有的 eps 都为 (-inf, inf)
